...
def generate_sample(simulator, visualize, save_data, root_path):
    ...
    sample_c, sample_ct = simulator.generate_sample()
    ...
    if TRAIN_DATA:
        ...
        if visualize:
            ...
            visualize_sample(sample=sample_ct,
                             simulator=simulator,
                             idcs_init=idcs_init,
                             idcs_bound=idcs_bound)
...
def write_data_to_file(root_path, simulator, sample_c, sample_ct):
    """
    ...
    :param sample_ct: The sample to be written to file (total concentration)
    """
    ...
    if TRAIN_DATA:
        ...
        np.save(file=os.path.join(data_path, "sample_ct.npy"),
                arr=sample_ct[:len(simulator.t)//4 + 1])
        ...
        np.save(file=os.path.join(data_path, "sample_ct.npy"), arr=sample_ct)
